 ...
 * Stolen from: https://github.com/Pkmmte/CircularImageView
 */
public class CircularImageView extends ImageView {
    ...
    public CircularImageView(Context context, AttributeSet attrs) {
        this(context, attrs, com.blueprint.watershed.R.styleable.CircularImageViewStyle_circularImageViewDefault);
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, com.blueprint.watershed.R.styleable.CircularImageView, defStyle, 0);

        // Check if border and/or border is enabled
        hasBorder = attributes.getBoolean(com.blueprint.watershed.R.styleable.CircularImageView_border, false);
        hasSelector = attributes.getBoolean(com.blueprint.watershed.R.styleable.CircularImageView_selector, false);

        // Set border properties if enabled
        if(hasBorder) {
            int defaultBorderSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setBorderWidth(attributes.getDimensionPixelOffset(com.blueprint.watershed.R.styleable.CircularImageView_border_width, defaultBorderSize));
            setBorderColor(attributes.getColor(com.blueprint.watershed.R.styleable.CircularImageView_border_color, Color.WHITE));
        }

        // Set selector properties if enabled
        if(hasSelector) {
            int defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setSelectorColor(attributes.getColor(com.blueprint.watershed.R.styleable.CircularImageView_selector_color, Color.TRANSPARENT));
            setSelectorStrokeWidth(attributes.getDimensionPixelOffset(com.blueprint.watershed.R.styleable.CircularImageView_selector_stroke_width, defaultSelectorSize));
            setSelectorStrokeColor(attributes.getColor(com.blueprint.watershed.R.styleable.CircularImageView_selector_stroke_color, Color.BLUE));
        }

        // Add shadow if enabled
        if(attributes.getBoolean(com.blueprint.watershed.R.styleable.CircularImageView_shadow, false))
            setShadow(true);
